...
import { validatePublicHost } from "./ssrf";
...
export async function fetchWithSecurityLimits(
  initialUrl: string,
  ...
  while (redirectCount <= maxRedirects) {
    ...
    if (typeof (res.headers as unknown as { getSetCookie?: () => string[] }).getSetCookie === "function") {
      setCookieHeaders = (res.headers as unknown as { getSetCookie: () => string[] }).getSetCookie();
    } else {
